// lib/reports.ts  (full replacement)
...
export interface CustomerReport {
  ...
  booking_trends: Array<{ date: string; bookings: number; avg_duration: string }>
  booking_patterns: {
...
class ReportsAPI {
  ...
  // Converts a ReportFilters object into a URLSearchParams query string.
  // KEY FIX: maps `startDate`/`endDate` → `start_date`/`end_date` so the
  // backend receives the names it expects, and skips undefined values.
  private buildQueryString(params?: ReportFilters): string {
    if (!params) return ''

    const sp = new URLSearchParams()

    // Period — only sent when no custom date range is set
    if (params.period && !params.startDate && !params.endDate) {
      sp.append('period', params.period)
    }
    ...
    // Optional filters
    if (params.minRevenue    != null) sp.append('min_revenue',    String(params.minRevenue))
    if (params.maxRevenue    != null) sp.append('max_revenue',    String(params.maxRevenue))
    if (params.minUtilization != null) sp.append('min_utilization', String(params.minUtilization))
    if (params.maxUtilization != null) sp.append('max_utilization', String(params.maxUtilization))
    if (params.vehicleBrand)  sp.append('vehicle_brand',  params.vehicleBrand)
    if (params.customerType)  sp.append('customer_type',  params.customerType)
    if (params.sortBy)        sp.append('sort_by',        params.sortBy)

    params.vehicleIds?.forEach(id => sp.append('vehicle_ids', id))
    params.customerIds?.forEach(id => sp.append('customer_ids', id))

    const qs = sp.toString()
    return qs ? `?${qs}` : ''
  }

  private async request<T>(endpoint: string, params?: ReportFilters): Promise<T> {
    const queryString = this.buildQueryString(params)
    const url = `${API_URL}${endpoint}${queryString}`

    console.log('📋 Report request:', url)
  ...
  async downloadPDF(type: ReportType, filters?: ReportFilters): Promise<void> {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || errorData.code === 'NO_TOKEN') {
        throw new Error('Authentication required. Please log in again to download reports.')
      }
  ...
  async downloadExcel(type: ReportType, filters?: ReportFilters): Promise<void> {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(errorData.message || `Failed to download Excel: ${response.statusText}`)
    }
