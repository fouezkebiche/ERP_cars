...
function HBar({ label, sublabel, value, max, color, rank, revenue, rentalsLabel, revenueLabel }: {
  label: string; sublabel?: string; value: number; max: number
  color: string; rank: number; revenue?: number; rentalsLabel: string; revenueLabel: string
}) {
    ...
    <div className={`relative rounded-xl p-4 transition-all ${rank === 1 ? "ring-1" : ""}`}
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{rentalsLabel}</p>
        </div>
      </div>
      ...
      {revenue !== undefined && (
        <p className="text-xs mt-2" style={{ color: "rgba(255,255,255,0.25)", fontFamily: "monospace" }}>
          {revenue.toLocaleString("fr-DZ")} DZD {revenueLabel}
        </p>
      )}
    </div>
  )
...
function CategoryChart({ data, rentalsLabel }: { data: { category: string; count: number; revenue: number }[]; rentalsLabel: string }) {
    ...
    <div className="flex items-center gap-6">
      <div className="shrink-0">
        <svg width={128} height={128} viewBox="0 0 128 128">
          ...
          <text x={cx} y={cy+11} textAnchor="middle" fill="rgba(255,255,255,0.3)" fontSize="7" fontFamily="monospace">{rentalsLabel}</text>
        </svg>
      </div>
...
export default function TrendingVehiclesPage() {
  const t = useTranslations("trendingVehicles")
  ...
  const subtitle = appliedCommune
    ? t("subtitleCommune", { commune: appliedCommune, wilaya: appliedWilaya, period: PERIOD_LABELS[period] })
    : appliedWilaya
      ? t("subtitleWilaya", { wilaya: appliedWilaya, period: PERIOD_LABELS[period] })
      : t("subtitleAll", { period: PERIOD_LABELS[period] })
    ...
    <div className="space-y-6 max-w-[1400px]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1">
            {(["7d","30d","90d","365d"] as const).map(p => (
              <PeriodBtn key={p} active={period===p} label={PERIOD_LABELS[p]} onClick={() => setPeriod(p)} />
            ))}
          </div>
          <select
            value={companyFilter}
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl px-4 py-3"
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-xs font-bold" style={{ color: "rgba(255,255,255,0.5)", fontFamily: "monospace" }}>{t("location")}</span>
        </div>
        ...
        >
          <option value="">{geoLoading ? t("loadingWilayas") : t("allWilayas")}</option>
        ...
        <select
          ...
          disabled={!selectedWilaya || geoLoading}
        ...
        >
          <option value="">
            {!selectedWilaya ? t("selectWilayaFirst") : t("allCommunesOf", { wilaya: selectedWilaya.name })}
          </option>
        ...
        <button
          onClick={handleApply}
          disabled={refreshing || geoLoading || !selectedWilaya}
      ...
      {error && (
        <div className="rounded-xl px-4 py-3 flex items-center gap-3"
          ...
          <button onClick={() => fetchData()} className="ml-auto text-xs underline" style={{ color: "#f87171" }}>{t("retry")}</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { labelKey: "kpi.totalRentals",    value: stats?.total_contracts,  suffix: "",    icon: <FileText size={15} />,  accent: "#f59e0b" },
          { labelKey: "kpi.totalRevenue",    value: stats?.total_revenue != null ? Math.round(stats.total_revenue).toLocaleString("fr-DZ") : "—", suffix: stats?.total_revenue != null ? " DZD" : "", icon: <DollarSign size={15} />, accent: "#34d399" },
          { labelKey: "kpi.activeVehicles",  value: stats?.unique_vehicles,  suffix: "",    icon: <Car size={15} />,       accent: "#818cf8" },
          { labelKey: "kpi.activeCompanies", value: stats?.unique_companies, suffix: "",    icon: <Building2 size={15} />, accent: "#60a5fa" },
        ].map(({ labelKey, value, suffix, icon, accent }) => (
          <div key={labelKey} className="rounded-xl p-4 relative overflow-hidden"
            ...
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{t(labelKey as any)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 rounded-2xl p-5" style={{ background: "#080810", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-sm font-black text-white" style={{ fontFamily: "monospace" }}>{t("ranking.title")}</h2>
            <span className="ml-auto text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(245,158,11,0.1)", color: "#f59e0b", fontFamily: "monospace" }}>
              {PERIOD_LABELS[period]}
            </span>
          </div>
          {loading
            ? <div className="space-y-3">{Array(6).fill(0).map((_,i) => <Skeleton key={i} className="h-20" />)}</div>
            : topVehicles.length === 0
              ? <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <p className="text-xs" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "monospace" }}>
                    {appliedWilaya
                      ? t("ranking.noDataLocation", { location: appliedCommune || appliedWilaya })
                      : t("ranking.noData")}
                  </p>
                </div>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl p-5" style={{ background: "#080810", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black text-white" style={{ fontFamily: "monospace" }}>{t("category.title")}</h3>
            </div>
            {loading
              ? <Skeleton className="h-32 w-full" />
              : categoryBreakdown.length > 0
                ? <CategoryChart data={categoryBreakdown} rentalsLabel={t("rentals")} />
                : <p className="text-xs text-center py-6" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "monospace" }}>{t("category.noData")}</p>
            }
          </div>
          ...
          {!loading && topVehicles.length >= 3 && (
            <div className="rounded-2xl p-5" style={{ background: "#080810", border: "1px solid rgba(255,255,255,0.07)" }}>
              <h3 className="text-sm font-black text-white mb-4" style={{ fontFamily: "monospace" }}>🏆 {t("podium.title")}</h3>
              <div className="space-y-2">
                {topVehicles.slice(0,3).map((v,i) => {
                  const medals = ["#f59e0b","#94a3b8","#cd7c3a"]
                  const podiumLabels = [t("podium.gold"), t("podium.silver"), t("podium.bronze")]
                  return (
                    <div key={v.vehicle_id} className="flex items-center gap-3 p-3 rounded-xl"
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs" style={{ color: medals[i]+"80", fontFamily: "monospace" }}>{podiumLabels[i]}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        <div className="rounded-2xl p-5" style={{ background: "#080810", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-black text-white" style={{ fontFamily: "monospace" }}>{t("leaders.title")}</h2>
              {appliedWilaya && (
          ...
          {loading ? (
            ...
            <p className="text-xs text-center py-8" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "monospace" }}>
              {appliedWilaya ? t("leaders.noDataLocation", { location: appliedCommune || appliedWilaya }) : t("leaders.noData")}
            </p>
          ) : (
              ...
              {visibleLeaders.map(c => (
                <div key={c.company_id} className="rounded-xl p-4 cursor-default"
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{t("rentals")}</span>
      ...
      {!loading && topVehicles.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: "#080810", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            <h2 className="text-sm font-black text-white" style={{ fontFamily: "monospace" }}>{t("table.title")}</h2>
          </div>
