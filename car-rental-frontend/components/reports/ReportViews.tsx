...
export function ExecutiveSummaryView({ report, formatCurrency }: WithFormatter<ExecutiveReport>) {
  ...
  const paymentChartData =
    report.revenue_breakdown?.by_method && report.revenue_breakdown.by_method.length > 0
      ? {
              ...
              columns={[
                {
                  key: "vehicle",
                  label: "Vehicle",
                  render: (_: any, row: any) => `${row.brand} ${row.model}`,
                },
                ...
                },
                {
                  key: "total_revenue",
                  label: "Revenue",
                  render: (v: number) => `${formatCurrency(v)} DZD`,
                },
              ]}
...
export function VehiclePerformanceView({ report, formatCurrency }: WithFormatter<VehicleReport>) {
  const [utilizationChartType, setUtilizationChartType] =
    useState<"bar" | "line">("bar")

  const utilizationChartData =
    report.top_performers && report.top_performers.length > 0
      ? {
          labels: report.top_performers
            .slice(0, 10)
            .map(
              (v: any) =>
          ...
          datasets: [
            {
              ...
              data: report.top_performers
                .slice(0, 10)
                .map((v: any) => v.utilization_rate),
              backgroundColor: report.top_performers
                .slice(0, 10)
                .map((v: any) => {
                  if (v.utilization_rate >= 80) return "#10b981"
                  if (v.utilization_rate >= 60) return "#3b82f6"
                  if (v.utilization_rate >= 40) return "#f59e0b"
                  return "#ef4444"
                }),
              borderColor: "#ffffff",
              borderWidth: 2,
  ...
  const profitChartData =
    report.top_performers && report.top_performers.length > 0
      ? {
          labels: report.top_performers
            .slice(0, 10)
            .map((v: any) => `${v.brand} ${v.model}`),
          datasets: [
            {
              label: "Revenue (DZD)",
              data: report.top_performers
                .slice(0, 10)
                .map((v: any) => v.total_revenue),
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (v: number) => (
                    <span
                      className={
                        v > 0 ? "text-red-600 font-semibold" : "text-green-600"
                      }
                    >
...
export function CustomerInsightsView({ report, formatCurrency }: WithFormatter<CustomerReport>) {
  const [segmentChartType, setSegmentChartType] =
    useState<"bar" | "doughnut">("bar")
  ...
  const segmentBarChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          label: (context: any) =>
            `${context.label}: ${context.parsed.y} customers`,
        },
      },
    },
    scales: {
  ...
  return (
    <>
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="p-6 rounded-lg border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-white shadow-sm">
          ...
          <div className="p-6 rounded-lg border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-white shadow-sm">
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="p-6 rounded-lg border-2 border-green-200 bg-gradient-to-br from-green-50 to-white shadow-sm">
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="p-6 rounded-lg border-2 border-amber-200 bg-gradient-to-br from-amber-50 to-white shadow-sm">
            <div className="flex items-center justify-between mb-2">
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant={
                    segmentChartType === "doughnut" ? "default" : "outline"
                  }
                  size="sm"
            ...
            <div className="h-80">
              {segmentChartType === "bar" ? (
                ...
                <Doughnut
                  ...
                  options={segmentDoughnutOptions}
                />
              )}
            </div>
          </CardContent>
            ...
            <CardContent>
              <div className="h-80">
                <Bar
                  ...
                  options={bookingPatternsOptions}
                />
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center p-6 bg-gradient-to-br from-blue-50 to-white rounded-lg border-2 border-blue-200">
              ...
              <p className="text-4xl font-bold text-blue-900">
                {report.retention_metrics.new_customers}
              </p>
            </div>
            <div className="text-center p-6 bg-gradient-to-br from-green-50 to-white rounded-lg border-2 border-green-200">
              ...
              <p className="text-4xl font-bold text-green-900">
                {report.retention_metrics.repeat_customers}
              </p>
            </div>
            <div className="text-center p-6 bg-gradient-to-br from-purple-50 to-white rounded-lg border-2 border-purple-200">
              ...
              <p className="text-4xl font-bold text-purple-900">
                {report.retention_metrics.retention_rate.toFixed(1)}%
              </p>
            </div>
          </div>
        </CardContent>
